...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        migration_queries = [
            ...
            "ALTER TABLE source_channels ADD COLUMN IF NOT EXISTS keep_link_keywords TEXT DEFAULT ''",
        ...
        # Forwarding rules (kaynak başına çoklu kural + çoklu hedef + link bazlı eşleşme)
        await conn.execute('''
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow('''
            ...
            (source_chat_id, target_chat_id, source_title, source_username,
             ...
             listen_type, trigger_keywords, keep_link_keywords, send_link_back, target_channel_id)
...
async def update_source_channel(
    ...
    async with pool.acquire() as conn:
        ...
        if trigger_keywords is not None:
            ...
            idx += 1
        if keep_link_keywords is not None:
            updates.append(f"keep_link_keywords = ${idx}")
            values.append(keep_link_keywords)
            idx += 1
...
async def get_active_rules_for_source(source_channel_id: int) -> List[Dict[str, Any]]:
    """Bir kaynak kanalın aktif kurallarını öncelik sırasına göre getir."""
    _check_pool()
    async with pool.acquire() as conn:
...
async def get_target_channels_by_ids(ids: List[int]) -> List[Dict[str, Any]]:
    """Verilen id listesine göre hedef kanalları getir (kural hedefleri için)."""
    _check_pool()
    if not ids:
        return []
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            'SELECT * FROM target_channels WHERE id = ANY($1::int[]) AND is_active = TRUE',
            list(ids)
        )
        return [dict(row) for row in rows]
...
async def get_today_post_count(source_channel_id: int) -> int:
    ...
    _check_pool()
    async with pool.acquire() as conn:
        # created_at timezone bilgisi olmadan kaydediliyor (naive UTC)
        # UTC+3 offset'i manuel uygula: created_at + 3 saat >= bugünün başlangıcı
        row = await conn.fetchrow('''
            SELECT COUNT(*) as count FROM posts
            WHERE source_channel_id = $1
            AND DATE(created_at + INTERVAL '3 hours') = DATE(NOW() AT TIME ZONE 'Europe/Istanbul')
            AND status = 'success'
        ''', source_channel_id)
