...
async function ensureTableStructure() {
  try {
    ...
    `);
    // Add keep_link_keywords column if it doesn't exist
    await query(`
      ALTER TABLE source_channels
      ADD COLUMN IF NOT EXISTS keep_link_keywords TEXT DEFAULT ''
    `);
  } catch {
    ...
    const result = await query(
      `INSERT INTO source_channels
       (source_chat_id, target_chat_id, target_channel_id, source_title, source_username,
        ...
        listen_type, trigger_keywords, keep_link_keywords, send_link_back)
...
export async function PUT(request: Request) {
  try {
    ...
    // source_chat_id (grup ID) degisebilir -> ozel olarak temizleyip guncelle
    if (
      updates.source_chat_id !== undefined &&
      updates.source_chat_id !== null &&
      String(updates.source_chat_id).trim() !== ''
    ) {
      const cleaned = String(updates.source_chat_id).replace(/[^0-9-]/g, '');
      if (cleaned.length > 0) {
        setClause.push(`source_chat_id = $${paramIndex}`);
        values.push(BigInt(cleaned).toString());
        paramIndex++;
      }
    }
