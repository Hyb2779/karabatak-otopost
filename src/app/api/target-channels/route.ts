...
export async function PUT(request: Request) {
  try {
    const body = await request.json();
    const { id, chat_id, title, username, is_active } = body;

    const setClause: string[] = [];
    const values: unknown[] = [id];
    let paramIndex = 2;

    if (chat_id !== undefined && String(chat_id).trim() !== '') {
      setClause.push(`chat_id = $${paramIndex}`);
      values.push(String(chat_id).trim());
      paramIndex++;
    }
    if (title !== undefined) {
    ...
    // Hedef kanalin chat_id'si degistiyse, ona bagli dinleme kanallarini da senkronla
    if (chat_id !== undefined && String(chat_id).trim() !== '') {
      await query(
        'UPDATE source_channels SET target_chat_id = $1 WHERE target_channel_id = $2',
        [String(chat_id).trim(), id]
      );
    }
